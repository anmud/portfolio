    ...
    isExpanded ? 'block' : 'hidden'
     ...
     <div className="text-lg sm:flex-grow py-10 ">
       ...
       <ul className="px-4   sm:items-center justify-end sm:px-0 sm:flex sm:flex-wrap ">
          ...
          <Link to={item.path} onClick={() => toggleNav()}  className="block text-lg font-medium uppercase tracking-wide text-white sm:text-black flex items-center border-b-4 border-transparent hover:border-yellow-500 sm:hover:border-black no-underline sm:block mr-4 mt-4 sm:mt-0 ">{item.text}</Link>
           </li>
